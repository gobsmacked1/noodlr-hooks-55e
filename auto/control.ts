...
export async function toggleSelectedCombatantAutomation(): Promise<void> {
  ...
  if (combatants.length === 0) {
    ui.notifications?.warn(game.i18n.localize("NOODLR.Combat.SelectToken"));
    return;
  }
  ...
  for (const combatant of combatants) {
    ...
    if (isPC) {
      ui.notifications?.warn(
        game.i18n.format("NOODLR.Combat.IsPCNamed", { name: combatant.name ?? "?" }),
      );
      continue;
    }
  ...
  if (enabled.length > 0) {
    ui.notifications?.info(
      game.i18n.format("NOODLR.Combat.AutomationOn", { names: enabled.join(", ") }),
    );
  }
  if (released.length > 0) {
    ui.notifications?.info(
      game.i18n.format("NOODLR.Combat.AutomationOff", { names: released.join(", ") }),
    );
  }
