...
import { isForcedMovementEnabled } from "../config";
...
import {
  ...
  type ForcedRule,
  type Trigger,
} from "../systems/dnd5e-forced-movement";
import {
  clearForcedHistory,
  dragAlong,
  isForcedMovement,
...
async function dragCaptives(moverDoc: any, movement: any): Promise<void> {
  ...
  for (const placeable of (canvas as any)?.tokens?.placeables ?? []) {
    ...
    const result = await dragAlong(
      ...
      dy,
      moverDoc,
      game.i18n.localize("NOODLR.Combat.Forced.Grappled"),
    );
    if (result.moved > 0) {
...
function wireUndoButton(_message: unknown, html: unknown): void {
  ...
  button.addEventListener("click", async () => {
    ...
    ui.notifications?.info(
      game.i18n.format("NOODLR.Combat.Forced.Restored", { count: String(restored) }),
    );
  });
}
