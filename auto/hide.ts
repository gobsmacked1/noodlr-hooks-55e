...
export async function hideSelected(options: { force?: boolean } = {}): Promise<void> {
  ...
  if (selected.length === 0) {
    ui.notifications?.warn(game.i18n.localize("NOODLR.Combat.Hide.NoSelection"));
    return;
  }
  ...
  for (const token of selected) {
    ...
    lines.push(
      result.hidden
        ? `<strong>${name}</strong> ${game.i18n.format("NOODLR.Combat.Hide.Hidden", {
            dc: String(result.total),
          })}`
        : `<strong>${name}</strong> ${game.i18n.format("NOODLR.Combat.Hide.Failed", {
