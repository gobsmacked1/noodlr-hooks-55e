...
import {
  ...
  midiOwnsDying,
  usesDeathSaves,
} from "../systems/dnd5e-dying";
import { isDnd5e } from "../systems/dnd5e-rewards";
import { hasStatus } from "../systems/dnd5e-conditions";
...
async function becomeDead(actor: any, reason: string, before: PendingDamage): Promise<void> {
  ...
  await announce(
    actor,
    game.i18n.format("NOODLR.Combat.Dying.Dead", {
...
async function becomeUnconscious(actor: any, before: PendingDamage): Promise<void> {
  ...
  await announce(
    actor,
    game.i18n.format("NOODLR.Combat.Dying.Unconscious", {
      name: String(actor.name ?? "Someone"),
    }),
    true,
  );
...
async function addDeathFailures(
  ...
  await announce(
    actor,
    game.i18n.format("NOODLR.Combat.Dying.Failure", {
      name: String(actor.name ?? "Someone"),
      add: String(add),
  ...
  if (next >= 3) {
    ...
    await becomeDead(
      actor,
      game.i18n.localize("NOODLR.Combat.Dying.Reason.ThreeFailures"),
      snap,
    );
  }
}
...
async function resolveAppliedDamage(actor: any): Promise<void> {
  ...
  if (isInstantDeath(oldHp, oldTemp, maxHp, amount)) {
    if (!hasStatus(actor, "dead")) {
      await becomeDead(
        actor,
        game.i18n.localize("NOODLR.Combat.Dying.Reason.Massive"),
        before,
      );
    }
    return;
  ...
  if (oldHp <= 0) {
    ...
    if (!saves) {
      ...
      if (!hasStatus(actor, "dead")) {
        await becomeDead(
          actor,
          game.i18n.localize("NOODLR.Combat.Dying.Reason.ZeroHp"),
          before,
        );
      }
      return;
...
async function onHpChanged(actor: any, changes: { hp: number; temp: number; total: number }, userId: string): Promise<void> {
  ...
  if (changes.total > 0 && newHp > 0) {
    ...
    await announce(
      actor,
      game.i18n.format("NOODLR.Combat.Dying.Revived", {
        name: String(actor.name ?? "Someone"),
      }),
      true,
    );
...
async function onPostDeathSave(
  ...
  try {
    ...
    if (
      ...
      await announce(
        actor,
        game.i18n.format("NOODLR.Combat.Dying.Stable", {
          name: String(actor.name ?? "Someone"),
        }),
        false,
      );
...
function wireUndoButton(_message: unknown, html: unknown): void {
  ...
  button.addEventListener("click", async () => {
    ...
    ui.notifications?.info(
      game.i18n.format("NOODLR.Combat.Dying.Restored", { count: String(restored) }),
    );
  });
}
