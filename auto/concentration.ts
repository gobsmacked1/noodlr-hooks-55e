...
// save is built right, and the verdict is discarded. See `systems/dnd5e-concentration.ts` for the
// citations.
//
...
import {
  ...
  type SaveVerdict,
} from "../systems/dnd5e-concentration";
...
function reasonText(cause: "dead" | "incapacitated" | "zero"): string {
  const key =
    cause === "dead"
      ? "NOODLR.Combat.Concentration.Reason.Dead"
      : cause === "incapacitated"
        ? "NOODLR.Combat.Concentration.Reason.Incapacitated"
        : "NOODLR.Combat.Concentration.Reason.Zero";
...
async function endAll(actor: any, reason: string): Promise<void> {
  ...
  try {
    ...
    await announce(
      actor,
      game.i18n.format("NOODLR.Combat.Concentration.Lost", {
...
function onConcentrationRolled(rolls: any, data: { subject?: any }): void {
  ...
  const reason =
    verdict.total !== null && verdict.dc !== null
      ? game.i18n.format("NOODLR.Combat.Concentration.Reason.FailedRoll", {
          total: String(verdict.total),
          dc: String(verdict.dc),
        })
      : game.i18n.localize("NOODLR.Combat.Concentration.Reason.Failed");
