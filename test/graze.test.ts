...
test("graze deals the ability modifier, in the weapon's own damage type", () => {
  const graze = grazeDamage(attack("graze"), {}, activity(4));
  assert.deepEqual(graze, { amount: 4, type: "slashing" });
});

test("no mastery, or a different one, deals nothing", () => {
  assert.equal(grazeDamage(attack(""), {}, activity(4)), null);
  assert.equal(grazeDamage(attack("topple"), {}, activity(4)), null);
});
...
test("a modifier of zero or less deals nothing rather than nothing loudly", () => {
  assert.equal(grazeDamage(attack("graze"), {}, activity(0)), null);
  assert.equal(grazeDamage(attack("graze"), {}, activity(-1)), null);
});
...
test("the modifier comes from the ability the attack actually used", () => {
  const graze = grazeDamage(attack("graze"), {}, activity(3, "piercing", "dex"));
...
test("the item's base damage type is read when the activity states none", () => {
  const item = { system: { damage: { base: { types: new Set(["bludgeoning"]) } } } };
  const bare = { ability: "str", actor: { system: { abilities: { str: { mod: 2 } } } } };
...
test("an unreadable damage type is left empty rather than guessed", () => {
  const bare = { ability: "str", actor: { system: { abilities: { str: { mod: 2 } } } } };
  assert.deepEqual(grazeDamage(attack("graze"), {}, bare), { amount: 2, type: "" });
});
