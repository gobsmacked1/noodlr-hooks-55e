...
const api: NoodlrHooksApi = {
  ...
  /** Who still owes initiative, and whether their walk and activity are frozen. */
  surveyInitiativeHold: () => surveyInitiativeHold(),
...
Hooks.once("ready", () => {
  ...
  registerOwedRolls();
  // A player who has not rolled initiative yet must not walk or act. Every client: the veto
  // is the player's own WASD / drag / activity, and a GM-only handler would not see it.
  registerInitiativeHold();
