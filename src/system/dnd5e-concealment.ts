...
// `rules/stealth.ts` pick between them. Nothing in `rules/` knows a spell name; it only knows the
 ...
 *
 * These only count when they physically stand in the way — see `core/screens.ts` for the geometry. The
 ...
 *
 * Called by `core/screens.ts` with whatever name it managed to resolve for a template, a region or a
