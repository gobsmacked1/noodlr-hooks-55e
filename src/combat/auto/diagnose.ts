...
export async function testMove(): Promise<Record<string, unknown> | undefined> {
  ...
  if (!origin) {
    console.warn("Noodlr Hooks | that token has no readable position");
    return undefined;
  }
  ...
  if (winner) {
    ...
    try {
      ...
      console.warn("Noodlr Hooks | could not return the token to its starting square:", err);
    }
  }
  ...
  console.log("Noodlr Hooks | movement test - copy everything below this line:");
