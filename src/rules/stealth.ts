...
// everyone else `rules/hide.ts` supplies the action the system never shipped.
//
...
// `system/dnd5e-concealment.ts`, and so are the things that see through them — truesight, See
...
async function declare(actor: any): Promise<void> {
  for (const token of tokensOf(actor)) {
    // `rules/hide.ts` writes the total itself, because it knows the roll it just made and does not depend
